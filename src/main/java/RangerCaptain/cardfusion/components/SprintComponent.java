...
public class SprintComponent extends AbstractComponent {
    public static final String ID = MainModfile.makeID(SprintComponent.class.getSimpleName());
    public static final String[] DESCRIPTION_TEXT = CardCrawlGame.languagePack.getUIString(ID).TEXT;
    public static final String[] CARD_TEXT = CardCrawlGame.languagePack.getUIString(ID).EXTRA_TEXT;

    public SprintComponent(float base) {
    ...
    public void onTrigger(ComponentAmountProvider provider, AbstractPlayer p, AbstractMonster m, List<AbstractComponent> captured) {
        Wiz.applyToSelf(new SprintPower(Wiz.adp(), provider.getAmount(this)));
    }

    @Override
    public AbstractComponent makeCopy() {
        return new SprintComponent(baseAmount);
    }
}
