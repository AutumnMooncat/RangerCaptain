...
import RangerCaptain.actions.StashCopyOfRandomCardsAction;
...
public class StashCardCopiesComponent extends AbstractComponent {
    public static final String ID = MainModfile.makeID(StashCardCopiesComponent.class.getSimpleName());
    ...
    public StashCardCopiesComponent(float base) {
        this(base, TargetPile.HAND, false, false);
    }

    public StashCardCopiesComponent(float base, TargetPile pile, boolean optional, boolean random) {
    ...
    public boolean shouldStack(AbstractComponent other) {
        if (other instanceof StashCardCopiesComponent) {
            if (((StashCardCopiesComponent) other).optional) {
                return true;
            }
            return !optional;
        }
    ...
    public void receiveStacks(AbstractComponent other) {
        if (other instanceof StashCardCopiesComponent) {
            optional |= ((StashCardCopiesComponent) other).optional;
            if (other.workingAmount > workingAmount) {
                pile = ((StashCardCopiesComponent) other).pile;
                random |= ((StashCardCopiesComponent) other).random;
            }
        }
    ...
    public void onTrigger(ComponentAmountProvider provider, AbstractPlayer p, AbstractMonster m, List<AbstractComponent> captured) {
        ...
        if (random) {
            addToBot(new StashCopyOfRandomCardsAction(group, 1, amount, c -> true, cards -> {
                for (AbstractCard card : cards) {
                    doCapturedActions(provider, p, m, captured, card);
                }
            }));
        } else {
            addToBot(new StashCopyOfCardsAction(group, 1, amount, optional, optional, c -> true, cards -> {
                for (AbstractCard card : cards) {
                    doCapturedActions(provider, p, m, captured, card);
                }
            }));
    ...
    public AbstractComponent makeCopy() {
        return new StashCardCopiesComponent(baseAmount, pile, optional, random);
    }
