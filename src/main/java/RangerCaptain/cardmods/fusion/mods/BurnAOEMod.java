...
public class BurnAOEMod extends AbstractExtraEffectFusionMod {
    public static final String ID = MainModfile.makeID(BurnAOEMod.class.getSimpleName());
    public static final String[] DESCRIPTION_TEXT = CardCrawlGame.languagePack.getUIString(ID).TEXT;
    public static final String[] CARD_TEXT = CardCrawlGame.languagePack.getUIString(ID).EXTRA_TEXT;

    public BurnAOEMod(int base) {
    ...
    public void onUse(AbstractCard card, AbstractCreature target, UseCardAction action) {
        if (!card.hasTag(CustomTags.MAGIC_BURN_AOE)) {
            Wiz.forAllMonstersLiving(mon -> Wiz.applyToEnemy(mon, new BurnedPower(mon, Wiz.adp(), val)));
        }
    }

    @Override
    public AbstractCardModifier makeCopy() {
        return new BurnAOEMod(baseVal);
    }
}
