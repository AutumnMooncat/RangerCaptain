...
public class TempStrengthMod extends AbstractExtraEffectFusionMod {
    public static final String ID = MainModfile.makeID(TempStrengthMod.class.getSimpleName());
    public static final String[] DESCRIPTION_TEXT = CardCrawlGame.languagePack.getUIString(ID).TEXT;
    public static final String[] CARD_TEXT = CardCrawlGame.languagePack.getUIString(ID).EXTRA_TEXT;

    public TempStrengthMod(int base) {
    ...
    public void onUse(AbstractCard card, AbstractCreature target, UseCardAction action) {
        if (!(card instanceof Squirey)) {
            Wiz.applyToSelf(new StrengthPower(Wiz.adp(), val));
            Wiz.applyToSelf(new LoseStrengthPower(Wiz.adp(), val));
        }
    }

    @Override
    public AbstractCardModifier makeCopy() {
        return new TempStrengthMod(baseVal);
    }
}
