...
public class ToxinMod extends AbstractExtraEffectFusionMod {
    public static final String ID = MainModfile.makeID(ToxinMod.class.getSimpleName());
    public static final String[] DESCRIPTION_TEXT = CardCrawlGame.languagePack.getUIString(ID).TEXT;
    public static final String[] CARD_TEXT = CardCrawlGame.languagePack.getUIString(ID).EXTRA_TEXT;

    public ToxinMod(int base) {
        super(ID, VarType.MAGIC, base);
    }

    @Override
    public float modifyBaseMagic(float magic, AbstractCard card) {
        if (card.hasTag(CustomTags.MAGIC_TOXIN) || card.hasTag(CustomTags.MAGIC_TOXIN_AOE)) {
    ...
    public void onUse(AbstractCard card, AbstractCreature target, UseCardAction action) {
        if (!card.hasTag(CustomTags.MAGIC_TOXIN) && !card.hasTag(CustomTags.MAGIC_TOXIN_AOE)) {
            Wiz.applyToEnemy((AbstractMonster) target, new ToxinPower(target, val));
        }
    }

    @Override
    public AbstractCardModifier makeCopy() {
        return new ToxinMod(baseVal);
    }
}
