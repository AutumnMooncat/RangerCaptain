...
public class PoisonAOEMod extends AbstractExtraEffectFusionMod {
    public static final String ID = MainModfile.makeID(PoisonAOEMod.class.getSimpleName());
    public static final String[] DESCRIPTION_TEXT = CardCrawlGame.languagePack.getUIString(ID).TEXT;
    public static final String[] CARD_TEXT = CardCrawlGame.languagePack.getUIString(ID).EXTRA_TEXT;

    public PoisonAOEMod(int base) {
    ...
    public void onUse(AbstractCard card, AbstractCreature target, UseCardAction action) {
        if (!card.hasTag(CustomTags.MAGIC_POISON_AOE)) {
            Wiz.forAllMonstersLiving(mon -> Wiz.applyToEnemy(mon, new PoisonPower(mon, Wiz.adp(), val)));
        }
    }

    @Override
    public AbstractCardModifier makeCopy() {
        return new PoisonAOEMod(baseVal);
    }
}
