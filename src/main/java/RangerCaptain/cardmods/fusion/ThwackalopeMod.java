...
public class ThwackalopeMod extends AbstractExtraEffectFusionMod {
    public static final String ID = MainModfile.makeID(ThwackalopeMod.class.getSimpleName());
    ...
    public void onUse(AbstractCard card, AbstractCreature target, UseCardAction action) {
        if (card.baseDamage == -1) {
            Wiz.attAfterBlock(new DamageAction(target, new DamageInfo(Wiz.adp(), val, DamageInfo.DamageType.NORMAL), AbstractGameAction.AttackEffect.BLUNT_HEAVY));
        }
    }

    @Override
    public AbstractCardModifier makeCopy() {
        return new ThwackalopeMod();
    }
}
