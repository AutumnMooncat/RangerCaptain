...
public class SouthpawMod extends AbstractExtraEffectFusionMod {
    public static final String ID = MainModfile.makeID(SouthpawMod.class.getSimpleName());
    ...
    public float modifyBaseMagic(float magic, AbstractCard card) {
        if (card.hasTag(CustomTags.MAGIC_RESONANCE)) {
            magic += AMOUNT2;
        }
        return magic;
    }
    ...
    public float modifyBaseSecondMagic(float magic, AbstractCard card) {
        if (card.hasTag(CustomTags.SECOND_MAGIC_RESONANCE)) {
            magic += AMOUNT2;
        }
        return magic;
    }
