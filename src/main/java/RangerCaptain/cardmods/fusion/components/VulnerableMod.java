...
public class VulnerableMod extends AbstractExtraEffectFusionMod {
    public static final String ID = MainModfile.makeID(VulnerableMod.class.getSimpleName());
    public static final String[] DESCRIPTION_TEXT = CardCrawlGame.languagePack.getUIString(ID).TEXT;
    public static final String[] CARD_TEXT = CardCrawlGame.languagePack.getUIString(ID).EXTRA_TEXT;

    public VulnerableMod(int base) {
    ...
    public float modifyBaseSecondMagic(float magic, AbstractCard card) {
        if (card.hasTag(CustomTags.SECOND_MAGIC_VULN) || card.hasTag(CustomTags.SECOND_MAGIC_VULN_AOE)) {
            magic += getModifiedBase(card);
        }
        return magic;
    }
    ...
    public String modifyDescription(String rawDescription, AbstractCard card) {
        if (!card.hasTag(CustomTags.MAGIC_VULN) && !card.hasTag(CustomTags.MAGIC_VULN_AOE) && !card.hasTag(CustomTags.SECOND_MAGIC_VULN) && !card.hasTag(CustomTags.SECOND_MAGIC_VULN_AOE)) {
    ...
    public void onUse(AbstractCard card, AbstractCreature target, UseCardAction action) {
        if (!card.hasTag(CustomTags.MAGIC_VULN) && !card.hasTag(CustomTags.MAGIC_VULN_AOE) && !card.hasTag(CustomTags.SECOND_MAGIC_VULN) && !card.hasTag(CustomTags.SECOND_MAGIC_VULN_AOE)) {
            Wiz.applyToEnemy((AbstractMonster) target, new VulnerablePower(target, val, false));
        }
    }

    @Override
    public AbstractCardModifier makeCopy() {
        return new VulnerableMod(baseVal);
    }
}
