        ...
        public static void plz(AbstractCard __instance, SpriteBatch sb) {
            if (__instance instanceof AbstractEasyCard) {
                if (((AbstractEasyCard) __instance).getGifOverlay() != null) {
                    ...
                    TextureRegion mask = getMask(__instance);
                    float maskX = __instance.current_x - mask.getRegionWidth() / 2.0F;
                    float maskY = __instance.current_y - mask.getRegionHeight() / 2.0F;
                    sb.draw(mask, maskX, maskY + 72.0F, mask.getRegionWidth()/2f, mask.getRegionHeight()/2f - 72.0F, mask.getRegionWidth(), mask.getRegionHeight(), __instance.drawScale * Settings.scale, __instance.drawScale * Settings.scale, __instance.angle);
        ...
        public static void plz(SingleCardViewPopup __instance, SpriteBatch sb, AbstractCard ___card) {
            if (___card instanceof AbstractEasyCard) {
                if (((AbstractEasyCard) ___card).getGifOverlay() != null) {
                    ...
                    sb.setBlendFunction(GL_DST_COLOR, GL_ZERO);
                    sb.setColor(Color.WHITE);
                    TextureRegion mask = getMask(___card);
